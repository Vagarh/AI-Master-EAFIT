...
## 2024-05-27 - [Fix Missing Error Handling Exposing Stack Traces in File Uploads]
**Vulnerability:** The `read_any` file upload function in `app.py` didn't catch explicit `ValueError` exceptions raised from security checks (e.g., maximum file sizes, disallowed extensions) nor any `Exception` from data processing, causing internal stack traces to be exposed in the UI.
**Learning:** In frameworks like Streamlit, unhandled exceptions generated by internal library implementations or explicit validations will bubble up and dump internal code paths directly to the user interface. By not handling specific exceptions, the application unnecessarily exposes implementation details.
**Prevention:** Wrap operations that process user input or files in structured `try...except` blocks. Expose safe, generalized error messages (`st.error`) to the user while logging detailed tracebacks securely using `app_logger`.

## 2026-05-06 - [Input Length Limits on User Fields]
